...
<<<<<<< HEAD
	...
	@GetMapping(value="/getconversatoinsofcompany", produces = "application/json")
		public ResponseEntity<ArrayList<ConversationTO>> getConversationsOfCompany(@PathVariable long id){
			...
			for(Event e : events) {
				if(e.getEventType().getType().equals("company_conversation"))
					convs.add(new ConversationTO(e.getEventInfoByType("start_datetime").getValue(),
		...
		for (Event event : events) {
			if (event.getEventType().getType().equals("company_conversation")) {
				...
				if (!event.getEventpersons().isEmpty()) {
					conversationTO.setPerson_company(
							event.getEventpersons().get(0).getPerson().getPDataFromType("firstname").getValue());
				}
